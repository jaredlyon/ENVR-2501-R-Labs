...
Alternate hypothesis: There is an positive association between age and swine flu prevalence, specifically in the young adult age demographic.
...
hist(swine.data$age, col = adjustcolor("red1"), xlim = c(0, 90), xlab = "Age", main = "Age Breakdown of Patients with Swine Flu")

## Create panel 2 -> age distribution of seasonal patients
seasonal.data <- subset(d1, flu == "seasonal", select = age)
hist(seasonal.data$age, col = adjustcolor("red1"), xlim = c(0, 90), xlab = "Age", main = "Age Breakdown of Patients with Seasonal Flu")
```

## 3.
The histogram distributions tell us that the swine flue targets individuals around the 15-70 age group, with most of this distribution being in the center. On the other hand, the seasonal flu only targets individuals ages below 20 and above 60. Notwithstanding minimal overlap between these age distributions, it appears that the two diseases do not target the same age demographics.
...
# Aggregate counts of seasonal data for the >18 age group
young.subset <- subset(d1, age < 18)
aggregate.young.data <- aggregate(age ~ flu, data = young.subset, FUN = length)
aggregate.young.data

# Aggregate counts of swine data for the 65< age group
old.subset <- subset(d1, age > 65)
aggregate.old.data <- aggregate(age ~ flu, data = old.subset, FUN = length)
aggregate.old.data

#combine aggregates into a single data frame
aggregate.total <- data.frame(aggregate.young.data, aggregate.middle.data, aggregate.old.data)
aggregate.total
```
