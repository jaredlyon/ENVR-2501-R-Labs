...
labels <- labels[order(names(labels))]
# Plot bars
bp <- barplot(means$`log10(score)`, beside = TRUE, ylim = c(0, 4), ylab = "HIV Score Means", col = c("white", "lightgray", "darkgray", "black"))
arrows(x0 = bp, y0 = means$`log10(score)` - stderrs$`log10(score)`, y1 = means$`log10(score)` + stderrs$`log10(score)`, angle = 90,
len = 0.1, code = 3, xpd = NA)
# Add labels for Males and Females
text(x = colMeans(bp)[1], y = -0.5, label = "Female", xpd = NA)
text(x = colMeans(bp)[2], y = -0.5, label = "Male", xpd = NA)
box()
...
# the treatments
legend(x = "topright", legend = c(XXXX, XXXX, XXXX, XXXX),
